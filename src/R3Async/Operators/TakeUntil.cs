...
public static partial class AsyncObservable
{
    extension<T>(AsyncObservable<T> source)
    {
        ...
        {
            TakeUntilStopSignalDelegate stopSignalSignal = notifyStop =>
            {
                var registration = cancellationToken.Register(x => ((Action<Result>)x).Invoke(Result.Success), 
                                                              notifyStop);
                return registration.ToAsyncDisposable();
            };

            return new TakeUntilFromRawSignal<T>(source, stopSignalSignal, TakeUntilOptions.Default);
        }
